# coding: utf-8

"""
    DUPR Backend APIs

    Internal RESTful APIs to access DUPR ratings, users and provide matches.

    The version of the OpenAPI document: v1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from dupr_backend.models.single_wrapper_club_response import SingleWrapperClubResponse

class TestSingleWrapperClubResponse(unittest.TestCase):
    """SingleWrapperClubResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> SingleWrapperClubResponse:
        """Test SingleWrapperClubResponse
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `SingleWrapperClubResponse`
        """
        model = SingleWrapperClubResponse()
        if include_optional:
            return SingleWrapperClubResponse(
                status = 'SUCCESS',
                message = 'Show this message to user.',
                result = dupr_backend.models.club_response.ClubResponse(
                    club_id = 1231231, 
                    club_name = 'Stillwater Pickleball', 
                    club_type = 2, 
                    media_url = '4684651981', 
                    address = 5156151658, 
                    short_address = 'Miami-Dade County, FL, US', 
                    club_member_count = 12, 
                    manifest = dupr_backend.models.league_content_response.LeagueContentResponse(
                        content_id = 45785789, 
                        header = '<h1>header</h1>', 
                        header_type = 'text/html', 
                        content = '<h1>content</h1>', 
                        content_type = 'text/html', 
                        footer = '<h1>footer</h1>', 
                        footer_type = 'text/html', ), 
                    short_description = dupr_backend.models.league_content_response.LeagueContentResponse(
                        content_id = 45785789, 
                        header = '<h1>header</h1>', 
                        header_type = 'text/html', 
                        content = '<h1>content</h1>', 
                        content_type = 'text/html', 
                        footer = '<h1>footer</h1>', 
                        footer_type = 'text/html', ), 
                    long_description = , 
                    attributes = dupr_backend.models.attributes.attributes(), 
                    role = dupr_backend.models.club_role_response.ClubRoleResponse(
                        role_id = 56, 
                        role = '', ), 
                    is_payment_setup = True, 
                    account_status = dupr_backend.models.account_status_response.AccountStatusResponse(
                        account_id = 6549864065, 
                        details_submitted = False, 
                        pending_requirement = False, 
                        errors = [{code=invalid_street_address, reason=The provided street address cannot be found. Please verify the street name and number are correct in "111 Boulevard Street", requirement=individual.address.line1}], ), 
                    model_type = 'ABSOLUTE', 
                    model_value = 1.337, 
                    currency_details = dupr_backend.models.currency_details_response.CurrencyDetailsResponse(
                        currency_code = 'USD', 
                        currency_symbol = '$', 
                        currency_name = 'US Dollar', 
                        min_limit = 100, ), 
                    created = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    requested_by = 56, 
                    club_join_type = 'INVITATION', 
                    pending_request_list = [1231231, 1231232, 1231233], 
                    distance_in_miles = 254, )
            )
        else:
            return SingleWrapperClubResponse(
        )
        """

    def testSingleWrapperClubResponse(self):
        """Test SingleWrapperClubResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
