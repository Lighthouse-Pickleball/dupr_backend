...
class UserSubscription(BaseModel):
    ...
    renewal_date: date = Field(alias="renewalDate")
    is_active: StrictBool = Field(alias="isActive")
    __properties: ClassVar[List[str]] = ["userId", "duprId", "cost", "billingPeriod", "startDate", "renewalDate", "isActive"]

    @field_validator('billing_period')
    def billing_period_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['WEEKLY', 'MONTHLY', 'ANNUAL']):
            raise ValueError("must be one of enum values ('WEEKLY', 'MONTHLY', 'ANNUAL')")
        return value
