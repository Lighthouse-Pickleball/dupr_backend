...
class ClubType(BaseModel):
    ...
    club_type: StrictStr = Field(alias="clubType")
    status: StrictStr
    __properties: ClassVar[List[str]] = ["clubTypeId", "clubType", "status"]

    @field_validator('club_type')
    def club_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ASSOCIATION', 'CLUB', 'COMPANY', 'FACILITY', 'GOVT_DEPARTMENT', 'NON_PROFIT', 'INDIVIDUAL']):
            raise ValueError("must be one of enum values ('ASSOCIATION', 'CLUB', 'COMPANY', 'FACILITY', 'GOVT_DEPARTMENT', 'NON_PROFIT', 'INDIVIDUAL')")
        return value
