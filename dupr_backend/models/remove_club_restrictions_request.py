...
class RemoveClubRestrictionsRequest(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["clubId", "restrictions"]

    @field_validator('restrictions')
    def restrictions_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(['UNKNOWN', 'MATCH_UPLOAD', 'MEMBER_INVITE_WEB']):
                raise ValueError("each list item must be one of ('UNKNOWN', 'MATCH_UPLOAD', 'MEMBER_INVITE_WEB')")
        return value
